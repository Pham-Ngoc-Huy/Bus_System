...
netstat -anp tcp | FIND /I "0.0.0.0:8090" >NUL
IF ERRORLEVEL 1 goto NOTFOUND
echo.
echo  Another server is running on port 8090 cannot run Apache server
echo.
goto END

:NOTFOUND
echo  Port 8090 is free - OK to run server
...
set apacheit=%Disk%:%apachepath%bin\Apache_9.exe -f %apachepath%conf\httpd.conf -d %apachepath%.
...
echo.
echo  To view site type http://localhost:8090 into a browser
goto :END
